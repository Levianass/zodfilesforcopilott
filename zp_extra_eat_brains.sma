...
public zp_fw_items_select_post(id, _itemid, ignorecost) {
    if (_itemid == _gItemID) {
        ...
        if(is_user_alive(id)) {
            ...
            new _RandomNum = random_num(1000,1500)
    
            set_user_health(id, get_user_health(id) + _RandomNum)
            ColorChat(id, TEAM_COLOR, "^x04[^x04ZoD *|^x04]^x01 You ate brains and got a drugged-up brain^x04 %d ^x01health.", _RandomNum)
        }
    }
    return ZP_ITEM_AVAILABLE;
} 
