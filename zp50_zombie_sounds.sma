...
new const sound_dragon_pain[][] = { "zombie_plague/nemesis_pain1.wav" , "zombie_plague/nemesis_pain2.wav" , "zombie_plague/nemesis_pain3.wav" }
...
new const sound_nightcrawler_pain[][] = { "zombie_plague/nemesis_pain1.wav" , "zombie_plague/nemesis_pain2.wav" , "zombie_plague/nemesis_pain3.wav" }
...
new Array:g_sound_dragon_pain
new Array:g_sound_assassin_pain
new Array:g_sound_nightcrawler_pain
...
public plugin_precache()
{
	...
	g_sound_dragon_pain = ArrayCreate(SOUND_MAX_LENGTH, 1)
	g_sound_assassin_pain = ArrayCreate(SOUND_MAX_LENGTH, 1)
	g_sound_nightcrawler_pain = ArrayCreate(SOUND_MAX_LENGTH, 1)
	...
	amx_load_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "DRAGON PAIN", g_sound_dragon_pain)
	amx_load_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "ASSASSIN PAIN", g_sound_assassin_pain)
	amx_load_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "NIGHTCRAWLER PAIN", g_sound_nightcrawler_pain)
	...
	if (ArraySize(g_sound_nemesis_pain) == 0)
	{
		...
		// Save to external file
		amx_save_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "DRAGON PAIN", g_sound_dragon_pain)
	}
	if (ArraySize(g_sound_dragon_pain) == 0)
	{
		for (index = 0; index < sizeof sound_dragon_pain; index++)
			ArrayPushString(g_sound_dragon_pain, sound_dragon_pain[index])
	...
	if (ArraySize(g_sound_nightcrawler_pain) == 0)
	{
		for (index = 0; index < sizeof sound_nightcrawler_pain; index++)
			ArrayPushString(g_sound_nightcrawler_pain, sound_nightcrawler_pain[index])
		
		// Save to external file
		amx_save_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "NIGHTCRAWLER PAIN", g_sound_nightcrawler_pain)
	}
	...
	// Dragon Class loaded?
	 else if (LibraryExists(LIBRARY_DRAGON, LibType_Library))
	{
		for (index = 0; index < ArraySize(g_sound_dragon_pain); index++)
		{
			ArrayGetString(g_sound_dragon_pain, index, sound, charsmax(sound))
			precache_sound(sound)
		}
	}	
	...
	// Nightcrawler Class loaded?
	 else if (LibraryExists(LIBRARY_NIGHTCRAWLER, LibType_Library))
	{
		for (index = 0; index < ArraySize(g_sound_nightcrawler_pain); index++)
		{
			ArrayGetString(g_sound_nightcrawler_pain, index, sound, charsmax(sound))
			precache_sound(sound)
		}
	}		
...
public fw_EmitSound(id, channel, const sample[], Float:volume, Float:attn, flags, pitch)
{
	...
	if (get_pcvar_num(cvar_zombie_sounds_pain))
	{
		...
		if (sample[7] == 'b' && sample[8] == 'h' && sample[9] == 'i' && sample[10] == 't')
		{
			...
			{
				ArrayGetString(g_sound_dragon_pain, random_num(0, ArraySize(g_sound_dragon_pain) - 1), sound, charsmax(sound))
				emit_sound(id, channel, sound, volume, attn, flags, pitch)
				return FMRES_SUPERCEDE;
			}
			// Assassin Class loaded?
			...
			{
				ArrayGetString(g_sound_nightcrawler_pain, random_num(0, ArraySize(g_sound_nightcrawler_pain) - 1), sound, charsmax(sound))
				emit_sound(id, channel, sound, volume, attn, flags, pitch)
				return FMRES_SUPERCEDE;
			}
