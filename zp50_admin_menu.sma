...
native zp_classes_menu_show(id)
...
show_menu_admin(id)
{
	...
	// 3. Start Game Mode command
	if (userflags & read_flags(g_access_start_game_mode))
		len += formatex(menu[len], charsmax(menu) - len, "\r3.\w %L^n", id, "MENU_ADMIN_START_GAME_MODE")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d3. %L^n", id, "MENU_ADMIN_START_GAME_MODE")

	// 4. Classes Menu Command
	if (userflags & read_flags(g_access_start_game_mode))
		len += formatex(menu[len], charsmax(menu) - len, "\r4.\w Open Classes Menu^n", id)
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d4. Open Classes Menu^n", id)
			...
			case ACTION_INFECT_CURE: // Infect/Cure command
			{
				if (zp_core_is_zombie(player))
				{
					if ((userflags & read_flags(g_access_make_human)) && is_user_alive(player))
						formatex(menu, charsmax(menu), "%s \r[%L]", player_name, id, (LibraryExists(LIBRARY_NEMESIS, LibType_Library) && zp_class_nemesis_get(player)) ? "CLASS_NEMESIS" : (LibraryExists(LIBRARY_ASSASSIN, LibType_Library) && zp_class_assassin_get(player)) ? "CLASS_ASSASSIN" : (LibraryExists(LIBRARY_WINOS, LibType_Library) && zp_class_winos_get(player)) ? "CLASS_WINOS" : "CLASS_ZOMBIE")
					else
						formatex(menu, charsmax(menu), "\d%s [%L]", player_name, id, (LibraryExists(LIBRARY_NEMESIS, LibType_Library) && zp_class_nemesis_get(player)) ? "CLASS_NEMESIS" : (LibraryExists(LIBRARY_ASSASSIN, LibType_Library) && zp_class_assassin_get(player)) ? "CLASS_ASSASSIN" : (LibraryExists(LIBRARY_WINOS, LibType_Library) && zp_class_winos_get(player)) ? "CLASS_WINOS" : "CLASS_ZOMBIE")
				}
				else
				{
...
public menu_admin(id, key)
{
	...
	switch (key)
	{
		...
		case ACTION_CLASSES_MENU: // Classes Menu command
		{
			if (userflags & read_flags(g_access_start_game_mode))
				zp_classes_menu_show(id)
			else
			{
				zp_colored_print(id, "%L", id, "CMD_NOT_ACCESS")
				show_menu_admin(id)
