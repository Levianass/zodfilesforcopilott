/*================================================================================
    ...
    ---------------------------
    -*- [ZP] Item: Antidote -*- (ZoD aka Zombies on Drugs)
    ---------------------------
...
// New variables to track Zombie Bomb & Zombie Darkness usage
new g_ItemID
...
new bool:g_HasUsedZombieBomb[33]
new bool:g_HasUsedZombieDarkness[33]

public plugin_init()
{
    register_plugin("[ZP] Item: Antidote (ZoD aka Zombies on Drugs)", ZP_VERSION_STRING, "ZP Dev Team")
...
public event_round_start()
{
    ...
    // Reset usage tracking for all players
    for (new i = 0; i < 33; i++)
    {
        g_HasUsedZombieBomb[i] = false
        g_HasUsedZombieDarkness[i] = false
    }
}

// Hook when Zombie Bomb is used
public zp_fw_items_select_post(id, itemid, ignorecost)
{
    if (itemid == zp_items_get_id("Zombie Bomb")) // Replace with actual ID if needed
        g_HasUsedZombieBomb[id] = true

    if (itemid == zp_items_get_id("Zombie Darkness")) // Replace with actual ID if needed
        g_HasUsedZombieDarkness[id] = true
...
public zp_fw_items_select_pre(id, itemid, ignorecost)
{
    ...
    // Restriction: Player used Zombie Bomb
    if (g_HasUsedZombieBomb[id])
        return ZP_ITEM_NOT_AVAILABLE;
    
    // Restriction: Player used Zombie Darkness
    if (g_HasUsedZombieDarkness[id])
        return ZP_ITEM_NOT_AVAILABLE;
