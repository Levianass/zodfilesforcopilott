...
{
	if (equal(module, LIBRARY_NEMESIS) || equal(module, LIBRARY_DRAGON) || equal(module, LIBRARY_NIGHTCRAWLER) || equal(module, LIBRARY_ASSASSIN))
...
set_on_fire(victim)
{
	...
	// Reduced duration for Dragon
	if (LibraryExists(LIBRARY_DRAGON, LibType_Library) && zp_class_dragon_get(victim))
	{
		// fire duration (dragon)
		g_BurningDuration[victim] += get_pcvar_num(cvar_grenade_fire_duration)
	}

	// Reduced duration for Nightcrawler
	if (LibraryExists(LIBRARY_NIGHTCRAWLER, LibType_Library) && zp_class_nightcrawler_get(victim))
	{
		// fire duration (nightcrawler)
		g_BurningDuration[victim] += get_pcvar_num(cvar_grenade_fire_duration)
	}
...
public burning_flame(taskid)
{
	...
	// Dragon Class loaded?
	if (!LibraryExists(LIBRARY_DRAGON, LibType_Library) || !zp_class_dragon_get(ID_BURN))
	{
		// Randomly play burning zombie scream sounds
		if (random_num(1, 20) == 1)
		{
	...
	if (!LibraryExists(LIBRARY_NIGHTCRAWLER, LibType_Library) || !zp_class_nightcrawler_get(ID_BURN))
	{
		// Randomly play burning zombie scream sounds
		if (random_num(1, 20) == 1)
		{
