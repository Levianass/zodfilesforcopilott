...
{	
	...
	{
		ArrayGetString(g_GameModeFileName, i, filename, charsmax(filename))
		unpause("ac",filename)
	}
	ArrayDestroy(g_GameModeName)
	ArrayDestroy(g_GameModeFileName)
}

public module_filter(const module[])
{
	if (equal(module, LIBRARY_NEMESIS) ||equal(module, LIBRARY_PREDATOR) ||  equal(module, LIBRARY_DRAGON) || equal(module, LIBRARY_ASSASSIN) || equal(module, LIBRARY_NIGHTCRAWLER) || equal(module, LIBRARY_SURVIVOR) || equal(module, LIBRARY_SNIPER) || equal(module, LIBRARY_KNIFER) || equal(module, LIBRARY_PLASMA))
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type)
{
	...
	if (g_AllowInfection && zp_core_is_zombie(attacker) && !zp_core_is_zombie(victim))
	{
		...
		// Predator shouldn't be infecting
		if (LibraryExists(LIBRARY_PREDATOR, LibType_Library) && zp_class_predator_get(attacker))
			return HAM_IGNORED;
		...
		// Nightcrawler shouldn't be infecting
		if (LibraryExists(LIBRARY_NIGHTCRAWLER, LibType_Library) && zp_class_nightcrawler_get(attacker))
			return HAM_IGNORED;
