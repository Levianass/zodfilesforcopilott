...
new cvar_human_unlimited_ammo, cvar_survivor_unlimited_ammo, cvar_sniper_unlimited_ammo, cvar_plasma_unlimited_ammo, cvar_knifer_unlimited_ammo
...
public plugin_init()
{
	...
	if (LibraryExists(LIBRARY_KNIFER, LibType_Library))
		cvar_knifer_unlimited_ammo = register_cvar("zp_knifer_unlimited_ammo", "1") // 1-bp ammo // 2-clip ammo

                // plasma Class loaded?
	if (LibraryExists(LIBRARY_PLASMA, LibType_Library))
		cvar_plasma_unlimited_ammo = register_cvar("zp_plasma_unlimited_ammo", "1") // 1-bp ammo // 2-clip ammo
...
public event_ammo_x(id)
{
	...
	{
		// Unlimited BP ammo enabled for knifer?
		if (get_pcvar_num(cvar_knifer_unlimited_ammo) < 1)
			return;
	}
                // Plasma Class loaded?
	else if (LibraryExists(LIBRARY_PLASMA, LibType_Library) && zp_class_plasma_get(id))
	{
		// Unlimited BP ammo enabled for plasma?
		if (get_pcvar_num(cvar_plasma_unlimited_ammo) < 1)
			return;
	}
	else
	{
	...
	else if (LibraryExists(LIBRARY_SNIPER, LibType_Library) && zp_class_sniper_get(msg_entity))
	{
		...
		if (get_pcvar_num(cvar_sniper_unlimited_ammo) < 2)
			return;
                ...
                // knifer Class loaded?
	else if (LibraryExists(LIBRARY_KNIFER, LibType_Library) && zp_class_knifer_get(msg_entity))
	{
		// Unlimited Clip ammo enabled for humans?
		if (get_pcvar_num(cvar_knifer_unlimited_ammo) < 2)
			return;
	}
                // plasma Class loaded?
	else if (LibraryExists(LIBRARY_PLASMA, LibType_Library) && zp_class_plasma_get(msg_entity))
	{
		// Unlimited Clip ammo enabled for humans?
		if (get_pcvar_num(cvar_plasma_unlimited_ammo) < 2)
			return;
	}
	else
	{
