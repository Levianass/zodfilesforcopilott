...
new g_MaxPlayers
new g_GameModeInfectionID, g_GameModeNemesisID, g_GameModeAssassinID, g_GameModeDragonID, g_GameModeNightcrawlerID, g_GameModeSurvivorID, g_GameModeSniperID, g_GameModeKniferID, g_GameModePlasmaID, g_GameModeWinosID
...
public plugin_init()
{
	...
	// Winos Class loaded?
	if (LibraryExists(LIBRARY_WINOS, LibType_Library))
		register_concmd("zp_winos", "cmd_winos", _, "<target> - Turn someone into a Winos", 0)
...
public plugin_natives()
{
	...
	register_native("zp_admin_commands_winos", "native_admin_commands_winos")
...
public native_admin_commands_winos(plugin_id, num_params)
{
	new id_admin = get_param(1)
...
// zp_winos [target]
public cmd_winos(id, level, cid)
{
	// Check for access flag - Make sniper
	if (!cmd_access(id, read_flags(g_access_make_winos), cid, 2))
		return PLUGIN_HANDLED;
	...
	// Target not allowed to be plasma
	if (zp_class_winos_get(player))
	{
		new player_name[32]
		get_user_name(player, player_name, charsmax(player_name))
		client_print(id, print_console, "[ZP] %L (%s).", id, "ALREADY_WINOS", player_name)
		return PLUGIN_HANDLED;
	}
	
	command_winos(id, player)
	return PLUGIN_HANDLED;
}
...
command_winos(id, player)
{
	...
	{
		// plasma mode disabled
		if (g_GameModeWinosID == ZP_INVALID_GAME_MODE)
		{
			zp_colored_print(id, "%L", id, "CMD_ONLY_AFTER_GAME_MODE")
			return;
		}
		...
		// Start plasma game mode with this target player
		if (!zp_gamemodes_start(g_GameModeWinosID, player))
		{
			zp_colored_print(id, "%L", id, "GAME_MODE_CANT_START")
			return;
		}
	...
	if (cvar_amx_show_activity)
	{
		switch (get_pcvar_num(cvar_amx_show_activity))
		{
			case 1: zp_colored_print(0, "ADMIN - %s %L", player_name, LANG_PLAYER, "CMD_WINOS")
			case 2: zp_colored_print(0, "ADMIN %s - %s %L", admin_name, player_name, LANG_PLAYER, "CMD_WINOS")
		}
	}
	...
	if (get_pcvar_num(cvar_log_admin_commands))
	{
		...
		get_user_authid(id, authid, charsmax(authid))
		get_user_ip(id, ip, charsmax(ip), 1)
		zp_log("ADMIN %s <%s><%s> - %s %L (Players: %d)", admin_name, authid, ip, player_name, LANG_SERVER, "CMD_WINOS", GetPlayingCount())
	}
}
