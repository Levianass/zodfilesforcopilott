...
new cvar_money_nemesis_ignore, cvar_money_dragon_ignore, cvar_money_survivor_ignore
new cvar_money_assassin_ignore, cvar_money_nightcrawler_ignore, cvar_money_sniper_ignore
new cvar_money_plasma_ignore, cvar_money_knifer_ignore
public plugin_init()
{
	...
	if (LibraryExists(LIBRARY_DRAGON, LibType_Library))
		cvar_money_dragon_ignore = register_cvar("zp_money_dragon_ignore", "0")
	...
	if (LibraryExists(LIBRARY_NIGHTCRAWLER, LibType_Library))
		cvar_money_nightcrawler_ignore = register_cvar("zp_money_nightcrawler_ignore", "0")
	...
	if (LibraryExists(LIBRARY_PLASMA, LibType_Library))
		cvar_money_plasma_ignore = register_cvar("zp_money_plasma_ignore", "0")

                // Knifer Class loaded?
	if (LibraryExists(LIBRARY_KNIFER, LibType_Library))
		cvar_money_knifer_ignore = register_cvar("zp_money_knifer_ignore", "0")
...
public fw_TakeDamage_Post(victim, inflictor, attacker, Float:damage, damage_type)
{
	...
	// Ignore money rewards for Dragon?
	if (LibraryExists(LIBRARY_DRAGON, LibType_Library) && zp_class_dragon_get(attacker) && get_pcvar_num(cvar_money_dragon_ignore))
		return;
	...
	// Ignore money rewards for Nightcrawler?
	if (LibraryExists(LIBRARY_NIGHTCRAWLER, LibType_Library) && zp_class_nightcrawler_get(attacker) && get_pcvar_num(cvar_money_nightcrawler_ignore))
		return;
	...
	if (LibraryExists(LIBRARY_SNIPER, LibType_Library) && zp_class_sniper_get(attacker) && get_pcvar_num(cvar_money_sniper_ignore))
		return;
                ...
                // Ignore money rewards for Plasma?
	if (LibraryExists(LIBRARY_PLASMA, LibType_Library) && zp_class_plasma_get(attacker) && get_pcvar_num(cvar_money_plasma_ignore))
		return;

                // Ignore money rewards for Knifer?
	if (LibraryExists(LIBRARY_KNIFER, LibType_Library) && zp_class_knifer_get(attacker) && get_pcvar_num(cvar_money_knifer_ignore))
		return;
