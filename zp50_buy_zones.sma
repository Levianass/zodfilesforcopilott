...
{
	if (equal(module, LIBRARY_SURVIVOR) || equal(module, LIBRARY_SNIPER) || equal(module, LIBRARY_KNIFER) || equal(module, LIBRARY_PLASMA) || equal(module, LIBRARY_AMMOPACKS))
...
{
	if (get_pcvar_num(cvar_buyzone_humans) && (!LibraryExists(LIBRARY_SURVIVOR, LibType_Library) || !zp_class_survivor_get(id)) && (!LibraryExists(LIBRARY_SNIPER, LibType_Library) || !zp_class_sniper_get(id)) && (!LibraryExists(LIBRARY_KNIFER, LibType_Library) || !zp_class_knifer_get(id)) && (!LibraryExists(LIBRARY_PLASMA, LibType_Library) || !zp_class_plasma_get(id)))
	{
	...
	{
		// Buyzone time ends when player is set to human/survivor/sniper/knifer/plasma
