...
public plugin_init()
{
	register_plugin("ZoD *| Main Menu", ZP_VERSION_STRING, "ZoD Dev Team")
...
{
	zp_colored_print(0, "==== ^x04Zombies On Drugs - Zombie Plague ^01====", ZP_VERSION_STR_LONG)
...
show_menu_main(id)
{
	...
	// Title
	len += formatex(menu[len], charsmax(menu) - len, "\yZombies On Drugs - Zombie Plague^n^n", ZP_VERSION_STR_LONG)
	...
	if (LibraryExists(LIBRARY_ADMIN_MENU, LibType_Library) && is_user_admin(id))
		...
		len += formatex(menu[len], charsmax(menu) - len, "\r7. VIP Menu^n^n")
	...
	case 1: // Extra Items
		{
			...
			if (LibraryExists(LIBRARY_ITEMS, LibType_Library))
			{
				...
				{
					zp_colored_print(id, "You can't buy extra items when round not started!")
					return PLUGIN_HANDLED;
				}
				if(!is_user_alive(id))
				{
		...
		case 6: // Menu override
		{
			if (is_user_admin(id))
			{
				if(zp_gamemodes_get_current() == ZP_NO_GAME_MODE)
				{
					zp_colored_print(id, "ViPs only can buy items after round started!")
					return PLUGIN_HANDLED;
				}
				if(zp_gamemodes_get_current() != zp_gamemodes_get_id("Infection Mode") && zp_gamemodes_get_current() != zp_gamemodes_get_id("Multi Infection Mode") && zp_gamemodes_get_current() != zp_gamemodes_get_id("Swarm Mode"))
				{
					zp_colored_print(id, "ViP menu unavailable at this time.")
					return PLUGIN_HANDLED;
				}
...
show_help(id)
{
	...
	len += formatex(motd[len], charsmax(motd) - len, "%L", id, "MOTD_INFO11", "Zombie Plague Mod", ZP_VERSION_STR_LONG, "ZoD Dev Team")
