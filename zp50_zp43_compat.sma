...
new g_GameModeInfectionID, g_GameModeMultiID, g_GameModeNemesisID, g_GameModeSurvivorID, g_GameModeSwarmID, g_GameModePlagueID, g_GameModeAssassinID, g_GameModeSniperID, g_GameModeLnjID, g_GameModeKniferID, g_GameModePlasmaID
new g_ModeStarted
...
public zp_fw_gamemodes_start(game_mode_id)
{
	...
	else if (game_mode_id == g_GameModeKniferID)
	{
		// Get knifer index
		new player_index = 1
		while ((!is_user_alive(player_index) || !zp_class_knifer_get(player_index)) && player_index <= g_MaxPlayers)
			player_index++
		
		if (player_index > g_MaxPlayers)
		{
			abort(AMX_ERR_GENERAL, "ERROR - knifer index not found!")
			player_index = 0
		}
		
		ExecuteForward(g_Forwards[FW_ROUND_STARTED], g_ForwardResult, MODE_KNIFER, player_index)
	}
	else if (game_mode_id == g_GameModePlasmaID)
	{
		// Get plasma index
		new player_index = 1
		while ((!is_user_alive(player_index) || !zp_class_plasma_get(player_index)) && player_index <= g_MaxPlayers)
			player_index++
		
		if (player_index > g_MaxPlayers)
		{
			abort(AMX_ERR_GENERAL, "ERROR - plasma index not found!")
			player_index = 0
		}
		
		ExecuteForward(g_Forwards[FW_ROUND_STARTED], g_ForwardResult, MODE_PLASMA, player_index)
	}
...
public plugin_natives()
{
	...
	register_native("zp_is_knifer_round", "native_is_knifer_round")
...
{
	if (equal(module, LIBRARY_NEMESIS) || equal(module, LIBRARY_SURVIVOR) || equal(module, LIBRARY_ASSASSIN) || equal(module, LIBRARY_SNIPER) || equal(module, LIBRARY_KNIFER) || equal(module, LIBRARY_PLASMA) || equal(module, LIBRARY_EXTRAITEMS) || equal(module, LIBRARY_FLASHLIGHT) || equal(module, LIBRARY_AMMOPACKS) || equal(module, LIBRARY_GRENADE_FROST))
		return PLUGIN_HANDLED;
...
public native_is_knifer_round(plugin_id, num_params)
{
	return (zp_gamemodes_get_current() == g_GameModeKniferID);
}

public native_is_plasma_round(plugin_id, num_params)
{
	return (zp_gamemodes_get_current() == g_GameModePlasmaID);
}
